...
ITEMS = {
    ...
    "bronze_box": "Небольшой сундук без замка, что же там?"
}
...
ROOMS = {
    ...
    'trap_room': {
        'description': """Комната с хитрой плиточной полом. 
    ...
    'garden': {
        'description': "Заброшенный подземный сад.",
        'exits': {'south': 'hall', 'west': 'armory', 'north': 'observatory'},
        'items': ['glowing_mushroom'],
        'puzzle': ("""Сколько листьев у квадрата семилистника?""", '49|сорок девять')
    },
    'observatory': {
        'description': """Круглая комната с разбитым куполом.
        В центре стоит древний телескоп.""",
        'exits': {'south': 'garden', 'east': 'treasure_room'},
        'items': ['treasure_key'],
        'puzzle': (
            """Телескоп показывает созвездие. 
            На табличке выбито: "Сколько звезд в Малой Медведице?" 
            (ответ цифрой)""", '7|семь'
            )
    },
    'treasure_room': {
    ...
    'secret_passage': {
        'description': """Узкий, пыльный тоннель, освещенный тусклым свечением мхов.""",
